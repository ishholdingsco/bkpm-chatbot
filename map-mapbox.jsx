...
// Mapbox GL JS landing-hero map. Uses the user's custom Studio style and
// renders Kawasan Industri (industrial estate) + KEK polygons on top.

const { useEffect: useEffectMB, useRef: useRefMB, useState: useStateMB } = React;

// Mapbox Standard with the "faded" theme — a calmer, monochromatic look
// (uses style imports + config; per Mapbox Standard guide).
const MB_STANDARD_FADED = {
  version: 8,
  imports: [{
    id: 'basemap',
...
window.__MAPBOX_STANDARD_FADED = MB_STANDARD_FADED;
...
function MapboxMap({
  ...
  const mapRef = useRefMB(null);
  const [ready, setReady] = useStateMB(false);
  const [errored, setErrored] = useStateMB(false);
  ...
  useEffectMB(() => {
    ...
    try {
      map = new mapboxgl.Map({
        ...
        style: style || window.__MAPBOX_STYLE,
        center, zoom, bearing,
    ...
    mapRef.current = map;
    if (typeof window !== 'undefined') window.__mbMap = map;

    map.on('error', (e) => {
      // swallow tile errors; surface only fatal style errors
  ...
  return (
    <div style={{ width: '100%', height: '100%', position: 'relative', background: '#cfe6f2' }}>
      ...
      {errored && (
        <div style={{
          position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center',
          color: '#a55', fontFamily: 'IBM Plex Mono, monospace', fontSize: 11,
        }}>MAP STYLE FAILED TO LOAD</div>
      )}
    </div>
  );
}
...
function addMbOverlays(map, onPinClick) {
  // Industrial estate polygons (Kawasan Industri)
  ...
  map.addLayer({
    id: 'mb-industrial-fill', type: 'fill', source: 'mb-industrial',
    paint: { 'fill-color': '#f7b500', 'fill-opacity': 0.55 },
  });
  map.addLayer({
    id: 'mb-industrial-line', type: 'line', source: 'mb-industrial',
    paint: { 'line-color': '#b07900', 'line-width': 1.2 },
  });
  ...
  map.addLayer({
    id: 'mb-kek-fill', type: 'fill', source: 'mb-kek',
    paint: { 'fill-color': '#7e4dd9', 'fill-opacity': 0.30 },
  });
  map.addLayer({
    id: 'mb-kek-line', type: 'line', source: 'mb-kek',
  ...
  // Soft halo (subtle, no glow)
  map.addLayer({
    id: 'mb-featured-halo', type: 'circle', source: 'mb-featured',
    paint: {
      'circle-radius': 14,
      'circle-color': '#c44a36',
      'circle-opacity': 0.10,
  ...
  map.addLayer({
    id: 'mb-featured-hit', type: 'circle', source: 'mb-featured',
    paint: {
      'circle-radius': 20,
      'circle-color': '#000',
      'circle-opacity': 0.001, // invisible but interactive
    },
  });
  // Small white-ringed dot — calmer, less prominent
  map.addLayer({
    id: 'mb-featured-ring', type: 'circle', source: 'mb-featured',
    paint: {
      'circle-radius': 5,
      'circle-color': '#c44a36',
  ...
  // ── Interactivity: cursor + tooltip popup + click ──
  const popup = new mapboxgl.Popup({
    closeButton: false, closeOnClick: false,
    offset: 14, className: 'mb-tip',
  });
  const enter = (e) => {
  ...
  // Industrial polygon hover tooltip
...
// We use a fill-extrusion-free fill layer with a filter so it covers all countries
// EXCEPT Indonesia ('IDN'). Real coastlines, no rough rectangles.
function addCountryMask(map) {
  if (!map.getSource('mb-countries')) {
    map.addSource('mb-countries', {
      type: 'vector',
      url: 'mapbox://mapbox.country-boundaries-v1',
    });
  }
  // Insert below the first symbol layer so labels stay legible
  let firstSymbol;
  try {
  ...
  try {
    map.addLayer({
      ...
      paint: {
        'fill-color': '#e8e2d0',     // muted warm parchment
        'fill-opacity': 0.78,
      },
    }, firstSymbol);
...
Object.assign(window, { MapboxMap });
