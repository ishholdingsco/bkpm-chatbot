...
import { DATA, Avatar, BKPM, TopBar } from "@/components/ui";

const SECTORS = [
  ["Critical minerals", true], ["Renewable energy", true], ["Digital infra", false],
  ["EV battery", true], ["Manufacturing", false], ["Agriculture", false],
  ["Tourism", false], ["Healthcare", false],
];
const TICKETS = ["<$50M", "$50M–$250M", "$250M–$1B", ">$1B"];
const STAGES = ["Exploring", "Active diligence", "Already invested"];
...
export function OnboardingThesis() {
  return (
    <div className="frame col" style={{ background: "var(--surface)" }}>
      <TopBar
        showOrg={false}
        right={<span className="mono" style={{ fontSize: 11, color: "var(--ink-3)" }}>Step 2 of 3 · Skip ›</span>}
      />
      ...
      <div className="row grow" style={{ minHeight: 0 }}>
        ...
        <div className="col" style={{ width: 480, padding: "40px 48px", background: "var(--surface)", overflow: "auto" }}>
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 18 }}>
            {STAGES.map((s, i) => (
              <span key={s} className={"chip " + (i === 1 ? "chip-terra" : "")} style={{ cursor: "pointer", padding: "6px 12px", fontSize: 11 }}>{i === 1 && "✓ "}{s}</span>
            ))}
          </div>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <Link href="/onboarding" style={{ textDecoration: "none" }}>
              <button className="btn"><ArrowLeft size={14} strokeWidth={1.75} /> Back</button>
            </Link>
            <Link href="/workspace" style={{ textDecoration: "none", flex: 1 }}>
              <button className="btn btn-primary" style={{ width: "100%" }}>Continue · Match analysts <ArrowRight size={14} strokeWidth={1.75} /></button>
            </Link>
          </div>
        </div>
