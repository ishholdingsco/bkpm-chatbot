...
// The hero screen: active chat thread with sidebar + canvas rail
...
function Sidebar({ collapsed, onToggle }) {
  if (collapsed) {
    return (
      <div className="col" style={{ width: 56, borderRight: '1px solid var(--line)', background: 'var(--surface-2)', padding: '12px 8px', gap: 12, alignItems: 'center' }}>
        <Logo />
        ...
        {DATA.projects.map(p => (
          <div key={p.id} title={p.name} style={{
            ...
            background: p.active ? 'var(--terracotta-soft)' : 'var(--surface)',
            border: '1px solid ' + (p.active ? 'var(--terracotta)' : 'var(--line)'),
            fontFamily: 'IBM Plex Mono, monospace', fontSize: 10, fontWeight: 600,
            color: p.active ? 'var(--terracotta)' : 'var(--ink-2)',
            cursor: 'pointer',
      ...
      <div className="scroll col grow" style={{ padding: '12px 10px', gap: 2 }}>
        ...
        {DATA.projects.map(p => (
          <div key={p.id} style={{
            padding: '7px 10px',
            borderRadius: 6,
            background: p.active ? 'var(--terracotta-soft)' : 'transparent',
            cursor: 'pointer',
            position: 'relative',
          }}>
...
function PresenceRibbon({ onBookCall, onPullIn, show }) {
  ...
  return (
    <div style={{
      background: 'linear-gradient(to right, var(--jade-soft), var(--surface-2))',
...
// ─────────────────────────────────────────────────────────────
// Floating analyst chip (alternative / additional placement)
// ─────────────────────────────────────────────────────────────
  ...
  if (isSuggest) {
    return (
      <div style={{
        ...
        borderRadius: 6, padding: '12px 14px', maxWidth: 640,
      }}>
        <div className="label" style={{ color: 'var(--terracotta)', marginBottom: 4 }}>● BKPM AI · suggested handoff</div>
        <div style={{ fontSize: 13, lineHeight: 1.55 }}>{turn.text}</div>
  ...
  return (
    <div className="col" style={{ alignItems: isUser ? 'flex-end' : 'flex-start', gap: 4, maxWidth: '100%' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11 }}>
        ...
        <span style={{ fontWeight: 600 }}>{turn.name}</span>
        <span className="mono" style={{ fontSize: 9, color: 'var(--ink-4)' }}>{turn.time}</span>
        {isUser && <Avatar name={DATA.user.short} color={DATA.user.color} size="sm" />}
      </div>
      <div style={{
        fontSize: 13.5, lineHeight: 1.55, maxWidth: 640,
      ...
      {turn.cite && (
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 2 }}>
          ...
          {turn.pin && (
            <span className="mono" style={{ fontSize: 10, color: 'var(--jade)' }}>↗ pinned to canvas: {turn.pin}</span>
          )}
        </div>
      )}
    </div>
...
// Composer
// ─────────────────────────────────────────────────────────────
function Composer({ onReferHuman }) {
  return (
    <div style={{ borderTop: '1px solid var(--line)', padding: '12px 20px 14px', background: 'var(--surface-2)' }}>
      <div className="card" style={{ padding: '10px 12px' }}>
        <div style={{ fontSize: 13, color: 'var(--ink-4)', minHeight: 38, lineHeight: 1.5 }}>
          Ask about regulations, structure, comps — or @mention an analyst…
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 4 }}>
          ...
          <button className="btn btn-sm btn-jade">Send</button>
        </div>
      </div>
    </div>
  );
...
// ─────────────────────────────────────────────────────────────
// Canvas rail (artifacts) — collapsible / popout
// ─────────────────────────────────────────────────────────────
    ...
    <div className="col" style={{
      ...
      <div className="scroll col grow" style={{ padding: 12, gap: 10 }}>
        <div style={{ fontSize: 11, color: 'var(--ink-3)', lineHeight: 1.4 }}>
          Pinned by AI + analyst as the thread evolves. Edits sync to project memory.
        </div>
        ...
        <div style={{
          padding: 10, border: '1px dashed var(--line-strong)',
          borderRadius: 6, textAlign: 'center', fontSize: 11, color: 'var(--ink-3)',
          marginTop: 4,
        }}>
          + Pin from chat or upload
        </div>
...
// ─────────────────────────────────────────────────────────────
function ActiveThread({ canvasMode, setCanvasMode, showPresence, onOpenHandoff, onOpenFocus }) {
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false);

  return (
    <div className="frame col">
      {/* top bar */}
      <div style={{ height: 44, borderBottom: '1px solid var(--line)', background: 'var(--surface)', display: 'flex', alignItems: 'center', padding: '0 16px', gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12 }}>
          <span className="mono" style={{ color: 'var(--ink-3)' }}>{DATA.org.name}</span>
      ...
      <div className="row grow" style={{ minHeight: 0 }}>
        ...
        <div className="col grow" style={{ minWidth: 0, position: 'relative', background: 'var(--surface)' }}>
          <PresenceRibbon
            show={showPresence}
            onBookCall={onOpenHandoff}
            onPullIn={onOpenHandoff}
          />
          <FloatingChip show={showPresence} onClick={onOpenHandoff} />

          {/* thread header */}
          <div style={{ padding: '20px 28px 12px', borderBottom: '1px solid var(--line)', background: 'var(--surface)' }}>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 12 }}>
              <h1 className="serif" style={{ fontSize: 22, fontWeight: 600, margin: 0, letterSpacing: '-0.01em' }}>
                {DATA.threads[0].name}
              </h1>
              <div className="grow" />
              <button className="btn btn-sm btn-ghost">⋯</button>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 6, alignItems: 'center' }}>
              <span className="mono" style={{ fontSize: 10, color: 'var(--ink-3)' }}>Started 26 Apr · 4 turns · 2 artifacts pinned</span>
          ...
          <div className="scroll col grow" style={{ padding: '20px 28px', gap: 18 }}>
            {DATA.turns.map((t, i) => <ChatTurn key={i} turn={t} />)}
          </div>

          <Composer onReferHuman={onOpenHandoff} />
        </div>
...
Object.assign(window, { ActiveThread });
