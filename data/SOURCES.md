...
| `minerals.json` | **Curated** flagship deposits (12 pts, multi-commodity: Ni, Cu, Au, bauxite, coal, tin). Editorial highlights — approximate belt/cluster centroids, NOT a downloaded dataset | Hand-curated by team; general refs ESDM Minerba + company disclosures |
| `mineral-logam.json` | 967 metal-mineral occurrence points — Nikel, Besi Laterit/Primer/Sedimen, Pasir Besi (GeoJSON FeatureCollection; `ore_t`/`metal_t` = largest reported resource category per point, tonnes) | ESDM / Badan Geologi — Keterdapatan Mineral Logam (peta 50K) |
| `iup-tambang.json` | 216 mining business permit (IUP/IUPK/PKP2B) concession **polygons** — Batubara, Nikel, Besi (GeoJSON; company, area_ha, stage, validity) | ESDM Minerba One Map (MOMI) — concession boundaries |
| `kawasan-hutan.json` | 5 forest-function **polygons** (one dissolved MultiPolygon per class: Hutan Lindung, Konservasi/KSA-KPA, Produksi Terbatas/Tetap/Konversi) — **thematic, raster-derived & simplified**; carries forest function only (NO province/area/company). Function labels inferred from the standard KLHK palette — verify before citing | Vectorized in-repo from a KLHK forest-status raster (`kawasan_hutan_zoom9.tif`, EPSG:4326) via `scripts/vectorize_hutan.py` (colour-quantize + morphological cleanup of seams/speckle) |
| `geologi-litologi.json` | 8 lithology-group **polygons** (one dissolved MultiPolygon per major rock type: Aluvium, Gunungapi, Batugamping, Klastik, Intrusi, Malihan, Ultramafik, Tak Terinci) — **thematic, raster-derived & simplified**; carries rock-type group only (NO province/area/formation/age). Groups are a best-effort keyword grouping of ~1,400 formations (many land in "Tak Terinci") — verify before citing | Vectorized in-repo from the ESDM Badan Geologi "Geologi Litologi" raster (`geologi_litologi_zoom8_fixed.tif`, EPSG:3857) via `scripts/build_geologi.py` (seamless nearest-group labelling + modal-filter denoise); formation→group via the ESDM MapServer renderer palette + `namobj`/`remark` attributes |
